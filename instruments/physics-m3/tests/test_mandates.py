...
class TestM4MapaUnico:
    def setup_method(self):
        ...
        self.mapa = MapaUnico(base_path=self.tmp)
    ...
    def test_validation_status(self):
        ...
        self.mapa.validate(eid, "PASS")
        assert self.mapa.get(eid).validation_status == "PASS"

    def test_summary(self):
        self.mapa.register("a", "x", {})
        s = self.mapa.summary()
        assert s["total_entries"] > 0
        assert "master_index.json" in s["index_path"]

    def test_index_saved(self):
        self.mapa.register("test", "obj", {"val": 1})
        idx_path = f"{self.tmp}/master_index.json"
